...
#makes a function # creates a seperate file for the function and opens it in R
usethis::use_r("add_one")

# to the function add_one add the following lines and save
add_one <- function (number){
  number +1
}

# makes a test for the function; creates folders tests ; opens a test file
usethis::use_test()
...
```{r}
# copy this to the test file test-add_one.R
```
...
important to be obvious than DRY (do not repeat yourself). A range of values and edge cases should
be included
...
## running tests
load packages: devtools::load_all(".")
...
devtools::test()
if asked, allow to create testing infrastructure
if the test does not work, read error, it might be that the testing file is not in the package subfolder "testthat" / and/or function in the package subfolder "R"
